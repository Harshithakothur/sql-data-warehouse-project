...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN sls_ship_date = 0 OR LEN(sls_ship_date) != 8 THEN NULL -- for future reference
		...
		SELECT 
			CASE WHEN cid LIKE '%NAS%' THEN SUBSTRING(cid,4, LEN(cid)-4+1)  --Remove 'NAS' Prefix ifPresent
					ELSE cid
			END as cid,
